...
const SelectionCard = ({ icon, label, isActive, onClick }: { icon: React.ReactNode, label: string, isActive: boolean, onClick: () => void }) => (
    <div
        ...
        ${isActive
            ? 'bg-yellow-400 border-white text-white shadow-xl scale-105'
            : 'bg-white/70 border-white/50 hover:bg-yellow-100'
        }`}
    >
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white/60 backdrop-blur-xl p-8 sm:p-12 rounded-3xl shadow-2xl border-4 border-white/50">
        <div className="text-center mb-10">
            <SparklesIcon className="w-16 h-16 mx-auto text-yellow-500 drop-shadow-lg" />
            <h2 className="text-4xl sm:text-5xl font-display mt-4 text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-yellow-500">魔法の呪文を唱えよう</h2>
            <p className="text-gray-600 mt-4 text-lg">あなたのアイデアから、世界にひとつだけの絵本が生まれます。</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
            <div>
                <label htmlFor="idea" className="block text-xl font-bold mb-3 text-gray-700">1. 物語のアイデアを入力</label>
                <textarea
                    id="idea"
                    value={idea}
                    onChange={(e) => setIdea(e.target.value)}
                    placeholder="例：うちの猫の「ソラ」が、お月様へ冒険に出かける話"
                    className="w-full h-32 p-4 border-4 border-yellow-300 rounded-2xl focus:ring-4 focus:ring-yellow-400/50 focus:border-yellow-500 transition-all duration-200 text-lg bg-slate-800 text-white placeholder-gray-400 shadow-inner"
                    required
                />
            </div>

            <div>
                <label className="block text-xl font-bold mb-3 text-gray-700">2. 主人公の画像をアップロード (任意)</label>
                 <label htmlFor="file-upload" className="flex flex-col items-center justify-center w-full h-64 border-4 border-yellow-300 border-dashed rounded-2xl cursor-pointer bg-yellow-50/50 hover:bg-yellow-100/70 transition-colors duration-200">
                    {imagePreview ? (
                        ...
                        <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center">
                            ...
                            <p className="mb-2 text-lg text-gray-500"><span className="font-semibold">クリックしてアップロード</span></p>
                            <p className="text-sm text-gray-500">ペットの写真や子供の絵など</p>
                        </div>
                    )}
            ...
            <div>
                <h3 className="text-xl font-bold mb-4 text-gray-700">3. テーマを選ぶ</h3>
            ...
            <div>
                <h3 className="text-xl font-bold mb-4 text-gray-700">4. アートスタイルを選ぶ</h3>
                <div className="flex flex-wrap justify-center gap-4">
            ...
            <div className="pt-6">
                <button type="submit" className="w-full flex items-center justify-center bg-yellow-400 hover:bg-yellow-500 text-white font-bold text-2xl py-5 px-6 rounded-full shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300 ease-in-out font-display tracking-wider button-boing border-4 border-white/50">
