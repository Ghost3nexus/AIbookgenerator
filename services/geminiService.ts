...
const createStorySchema = (pageCount: number) => ({
    type: Type.OBJECT,
    properties: {
        ...
        pages: {
            type: Type.ARRAY,
            description: `${pageCount}ページからなる物語のページ配列。`,
            items: {
                type: Type.OBJECT,
                properties: {
...
const handleApiError = (error: any, context: string): never => {
    ...
    if (errorMessage.includes('429') || errorMessage.includes('RESOURCE_EXHAUSTED')) {
        throw new Error('APIの利用上限に達したようです。時間をおいて試すか、ページ数を減らしてください。詳細はGoogle AI StudioのUsageページをご確認ください。');
    }
    if (errorMessage.includes('API key not valid')) {
        throw new Error('APIキーが無効です。設定モーダルから正しいキーを再入力してください。');
    }
    throw new Error(`AIとの通信中にエラーが発生しました (${context})。もう一度お試しください。`);
};
...
export async function generateStoryAndImages(
    ...
    try {
        ...
        const systemInstruction = `あなたは受賞歴のある児童文学作家であり、優しいイラストレーターです。ユーザーの断片的なアイデアを、4〜6歳の子どもを対象とした、心温まる絵本（表紙、物語${pageCount}ページ、あとがき）に変えるのがあなたの仕事です。物語は常にハッピーエンドにしてください。`;
...
`
        ...
        const storySchema = createStorySchema(pageCount);
        ...
        const imageResults = await Promise.all([coverImagePromise, ...pageImagePromises]);
        const [coverImageResult, ...pageImageResults] = imageResults;
